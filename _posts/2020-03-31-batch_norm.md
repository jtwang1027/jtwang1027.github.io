...
As another approach, we tested for gene set enrichment with the expectation of finding significant biological processes that would be unique to a specific cell / tissue type. We looked for enrichment of gene sets (using the Gene Ontology annotation) using [AUCell](https://www.bioconductor.org/packages/release/bioc/html/AUCell.html). Gene expression rankings are built for each cell. Next, enrichment of each gene set is plotted against each cell's gene expression ranking (the top 5% of expressed genes). The calculated AUC the proportion of genes in a gene set that are highly expressed (for a given cell).

![AUCell_gene_sets](https://user-images.githubusercontent.com/46359281/88752981-24d49f80-d129-11ea-911f-b525cfb4fcba.png)

Interestingly, several gene sets of biological relevance were significantly enriched. Previous work has identified IL-8 in the glioblastoma secretome, where it stimulates pro-angiogenic signals and endothelial permeability[(6)](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3447807/). This is further supported by the enrichment of the blood vessel remodeling gene set. In  addition, we identify 2 immune populations: T cells and antigen presenting cells (potenially microglia). Microglia are well-known drivers of brain tumor pathobiology and can make up 1/3 of the tumor[(7)](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7288606/) These results suggest that gene set enrichment can be used to identify aberrant pathways in tumor subpopulations cells, and can be used to study cell and pathway usage heterogeneity.
